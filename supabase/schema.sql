...
declare
  ...
  v_aviso text := '';
begin
  -- trim() evita o erro mais comum: espaço invisível colado junto do e-mail
  select id, email_confirmed_at
    into v_id, v_confirmado
  from auth.users
  where lower(email) = lower(trim(p_email));

  if v_id is null then
    return '❌ Não existe usuário com o e-mail "' || trim(p_email) ||
           '". Rode  select * from public.listar_contas();  para ver os e-mails cadastrados.';
  end if;

  -- 1) Confirma o e-mail (sem isso o login falha mesmo com a senha certa).
  --    Obs.: só mexemos em email_confirmed_at. A coluna confirmed_at é
  --    calculada sozinha pelo Supabase e não pode ser alterada na mão.
  if v_confirmado is null then
    update auth.users
       set email_confirmed_at = now()
     where id = v_id;
    v_aviso := ' (e-mail confirmado automaticamente)';
  end if;

  -- 2) Garante o perfil com papel de admin
  insert into public.profiles (id, email, nome, role)
  values (v_id, p_email, p_email, 'admin')
  on conflict (id) do update
    set role = 'admin', email = excluded.email;

  -- 3) Remove a "empresa" criada automaticamente para esta conta,
  --    desde que ela ainda não tenha vagas.
  delete from public.empresas e
  where e.user_id = v_id
    and not exists (select 1 from public.vagas v where v.empresa_id = e.id);

  return '✅ ' || p_email || ' agora é ADMINISTRADOR' || v_aviso ||
         '. Acesse /admin/login';
end;
$$;

-- ---------------------------------------------------------------------
-- 9.1 LISTAR AS CONTAS (ajuda a achar erro de digitação no e-mail)
--     Rode:  select * from public.listar_contas();
-- ---------------------------------------------------------------------
create or replace function public.listar_contas()
returns table (email text, confirmado text, papel text, criado_em timestamptz)
language sql
security definer
set search_path = public
as $$
  select
    u.email::text,
    case when u.email_confirmed_at is null then '❌ não' else '✅ sim' end,
    coalesce(p.role, '(sem perfil)')::text,
    u.created_at
  from auth.users u
...
-- ---------------------------------------------------------------------
-- 9.2 DIAGNÓSTICO: por que o login não está funcionando?
--     Rode:  select * from public.checar_conta('seu@email.com');
-- ---------------------------------------------------------------------
create or replace function public.checar_conta(p_email text)
returns table (item text, situacao text)
language plpgsql
security definer
set search_path = public
as $$
...
begin
  select u.id, u.email, u.email_confirmed_at
    into v_id, v_email_real, v_confirmado
  from auth.users u
  ...
  if v_id is null then
    return query select
      'Conta'::text,
      ('❌ Não existe usuário com o e-mail "' || p_email ||
       '". Confira se digitou igual ao cadastrado em Authentication > Users.')::text;
    return;
  end if;

  return query select 'E-mail cadastrado'::text, ('✅ ' || v_email_real)::text;

  if v_confirmado is null then
    return query select
      'E-mail confirmado'::text,
      '❌ NÃO — é por isso que o login falha! Rode: select public.promover_admin(''' || v_email_real || ''');'::text;
  else
    return query select 'E-mail confirmado'::text, '✅ Sim'::text;
  end if;

  select p.role into v_papel from public.profiles p where p.id = v_id;

  if v_papel is null then
    return query select
      'Perfil no banco'::text,
      '❌ Não existe. Rode: select public.promover_admin(''' || v_email_real || ''');'::text;
  elsif v_papel <> 'admin' then
    return query select
      'Permissão'::text,
      ('❌ É "' || v_papel || '", não admin. Rode: select public.promover_admin(''' || v_email_real || ''');')::text;
  else
    return query select 'Permissão'::text, '✅ Administrador'::text;
  end if;

  if v_confirmado is not null and v_papel = 'admin' then
    return query select
      'Resultado'::text,
      '🎉 Tudo certo! Se ainda não entrar, a senha está errada — redefina em Authentication > Users > (3 pontinhos) > Reset password.'::text;
  end if;
end;
$$;

-- =====================================================================
-- 10. SEGURANÇA DAS FUNÇÕES DE MANUTENÇÃO
--
--     As três funções acima (promover_admin, listar_contas, checar_conta)
--     leem e alteram dados sensíveis. Elas devem funcionar APENAS aqui no
--     SQL Editor — nunca pela internet. Por isso, revogamos o acesso dos
--     visitantes e dos usuários logados.
-- ---------------------------------------------------------------------
revoke all on function public.promover_admin(text) from public, anon, authenticated;
revoke all on function public.listar_contas()      from public, anon, authenticated;
revoke all on function public.checar_conta(text)   from public, anon, authenticated;

-- =====================================================================
-- 11. COMO CRIAR O SEU ADMINISTRADOR
--
...
--
--   Não sabe qual e-mail cadastrou?  select * from public.listar_contas();
-- =====================================================================
