...
function acharNome(texto: string, nomeArquivo: string) {
  ...
  // 3) Nome que apareça logo antes/depois do e-mail encontrado
  //    (ex.: "Pamela Joana Rosa  pamela@gmail.com")
  const doEmail = nomePertoDoEmail(texto);
  if (doEmail) return doEmail;

  // 4) Último recurso: o nome do arquivo — mas só se ele PARECER um nome.
  //    Arquivos como "04-07-2025 14-21-29.ANEXE SEU CURRÍCULO-.172329"
  //    não têm nome nenhum, e é melhor deixar vazio do que inventar.
...
/** Procura um nome de pessoa nas linhas vizinhas ao e-mail. */
function nomePertoDoEmail(texto: string) {
  const linhas = texto.split("\n").map(limpar);
  const iEmail = linhas.findIndex((l) => /[\w.+-]+@[\w.-]+\.\w{2,}/.test(l));
  if (iEmail === -1) return "";

  const vizinhas = [
    linhas[iEmail].replace(/[\w.+-]+@[\w.-]+\.\w{2,}/g, " "),
    linhas[iEmail - 1] ?? "",
    linhas[iEmail + 1] ?? "",
  ];

  for (const bruta of vizinhas) {
    const c = limpar(bruta.replace(/[^A-Za-zÀ-ÿ\s'.-]/g, " "));
    if (!c || NAO_E_NOME.test(c) || PALAVRAS_CARGO.test(c)) continue;
    const palavras = c.split(" ").filter((p) => p.length > 1);
    if (palavras.length < 2 || palavras.length > 5) continue;
    if (palavras.every((p) => /^[A-Za-zÀ-ÿ'.-]+$/.test(p))) {
      return titulo(palavras.join(" "));
    }
  }
  return "";
}
...
/**
 * Tenta tirar o nome do nome do arquivo — com rigor.
 * Só aceita se sobrar algo que realmente pareça nome de pessoa.
 */
function nomeDoArquivo(nomeArquivo: string) {
  // Palavras de sistema/formulário que aparecem em nomes automáticos
  const LIXO = new RegExp(
    [
      "anexe", "anexo", "seu", "sua", "aqui", "curr[ií]culo", "curriculum",
      "vitae", "\\bcv\\b", "resume", "documento", "arquivo", "scan",
      "digitalizado", "imagem", "image", "foto", "photo", "whatsapp",
      "screenshot", "captura", "de\\s+tela", "download", "copy", "c[óo]pia",
      "novo", "final", "atualizado", "versao", "vers[ãa]o", "form",
      "formul[áa]rio", "resposta", "envio", "upload", "untitled",
      "sem\\s+t[íi]tulo", "doc", "pdf", "img", "at\\b",
    ].join("|"),
    "gi",
  );

  const base = nomeArquivo
    .replace(/\.[^.]+$/, "")          // tira a extensão
    .replace(/\d/g, " ")              // tira números (datas, horas, ids)
    .replace(/[_\-.,()[\]{}+]+/g, " ") // separadores viram espaço
    .replace(LIXO, " ");

  const palavras = limpar(base)
    .split(" ")
    .filter((p) => p.length > 1 && /^[A-Za-zÀ-ÿ'.-]+$/.test(p));

  // Precisa de 2 a 5 palavras que pareçam nome
  if (palavras.length < 2 || palavras.length > 5) return "";

  const junto = palavras.join(" ");
  if (NAO_E_NOME.test(junto) || PALAVRAS_CARGO.test(junto)) return "";

  // Cada palavra precisa ter cara de nome próprio (tem vogal, tamanho ok)
  const parecemNomes = palavras.every(
    (p) => p.length >= 2 && p.length <= 20 && /[aeiouàáâãéêíóôõúAEIOU]/.test(p),
  );
  if (!parecemNomes) return "";
